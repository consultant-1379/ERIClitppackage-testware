...
public class LITPpackageTestRunner extends TorTestCaseHelper {
    
    Logger logger = Logger.getLogger(LITPpackageTestRunner.class);
    ...
    @TestId(id = "CXP9031119-1", title = "Upgrade ERIClitppackage rpm on the MS")
    @Test(groups={"ACCEPTANCE"})
    public void upgradeERIClitppackageRPM() throws TimeoutException, FileNotFoundException {

        rpmUpgradeOperator.initialise("package");
     ...
     * @PRIORITY HIGH
     */
    @TestId(id = "CXP9031119-2", title = "Run python test cases for ERIClitppackage package")
    @Test(groups={"CDB_REGRESSION", "ACCEPTANCE"})
    public void runERIClitppackageTests() {
     ...
     * @PRE Execution of test {@link #runERIClitppackageTests()}
     * @PRIORITY HIGH
     * @param className
     * @param name
     * @param failures
     */
    @TestId(id = "CXP9031119-3", title = "Parse xml outputs from python tests")
